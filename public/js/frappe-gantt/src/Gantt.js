...
export default function Gantt(element, tasks, config) {
  ...
  function set_gantt_dates() {
    if (view_is(["Quarter Day", "Half Day"])) {
      ...
      self.gantt_end = self.gantt_end
        .clone()
        .endOf("month")
        .add(1, "year");
    } else {
      self.gantt_start = self.gantt_start
        .clone()
        .startOf("month")
        .subtract(1, "month");
      self.gantt_end = self.gantt_end
  ...
  function set_scroll_position() {
    ...
    const scroll_pos =
      get_min_date().diff(self.gantt_start, "hours") /
        self.config.step *
  ...
  function make_grid_header() {
    ...
    self.canvas
      .rect(0, 0, header_width, header_height)
  ...
  function make_grid_rows() {
    ...
    for (let task of self.tasks) {
      // eslint-disable-line
      self.canvas
        .rect(0, row_y, row_width, row_height)
        .addClass("grid-row")
        .appendTo(rows);

      self.canvas
        .line(0, row_y + row_height, row_width, row_y + row_height)
        .addClass("row-line")
        .appendTo(lines);
  ...
  function make_dates() {
    for (let date of get_dates_to_draw()) {
      self.canvas
        .text(date.lower_x, date.lower_y, date.lower_text)
        .addClass("lower-text")
        .appendTo(self.element_groups.date);

      if (date.upper_text) {
        const $upper_text = self.canvas
          .text(date.upper_x, date.upper_y, date.upper_text)
          .addClass("upper-text")
          .appendTo(self.element_groups.date);

        // remove out-of-bound dates
        if (
          $upper_text.getBBox().x2 > self.element_groups.grid.getBBox().width
        ) {
          $upper_text.remove();
        }
      }
  ...
  function get_date_info(date, last_date, i) {
    ...
    const date_text = {
      ...
      "Quarter Day_upper":
        date.date() !== last_date.date() ? date.format("D MMM") : "",
      "Half Day_upper":
        date.date() !== last_date.date() ? date.format("D MMM") : "",
