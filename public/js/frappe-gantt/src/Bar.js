...
export default function Bar(gt, task) {
  ...
  function prepare_values() {
    ...
    self.duration =
      (self.task._end.diff(self.task._start, "hours") + 24) / gt.config.step;
    self.width = gt.config.column_width * self.duration;
    self.progress_width =
      gt.config.column_width * self.duration * (self.task.progress / 100) || 0;
    self.group = gt.canvas.group().addClass("bar-wrapper");
    self.bar_group = gt.canvas
      .group()
      .addClass("bar-group")
      .appendTo(self.group);
    self.handle_group = gt.canvas
      .group()
      .addClass("handle-group")
      .appendTo(self.group);
  }
  ...
  function draw_label() {
    gt.canvas
      .text(self.x + self.width / 2, self.y + self.height / 2, self.task.name)
      .addClass("bar-label")
      .appendTo(self.bar_group);
  ...
  function draw_resize_handles() {
    ...
    if (self.task.progress && self.task.progress < 100) {
      gt.canvas
        .polygon(get_progress_polygon_points())
  ...
  function show_details() {
    ...
    self.details_box = popover_group.select(
      `.details-wrapper[data-task='${self.task.id}']`
    );

    if (!self.details_box) {
      self.details_box = gt.canvas
        .group()
      ...
      gt.canvas
        ...
        .addClass("details-body")
        .appendTo(self.details_box);
      const f = gt.canvas.filter(Snap.filter.shadow(0, 1, 1, "#666", 0.6));
  ...
  function compute_start_end_date() {
    ...
    const new_start_date = gt.gantt_start
      .clone()
      .add(x_in_units * gt.config.step, "hours");
    const width_in_units = bar.getWidth() / gt.config.column_width;
    const new_end_date = new_start_date
      .clone()
      .add(width_in_units * gt.config.step, "hours");
  ...
  function compute_x() {
    let x =
      self.task._start.diff(gt.gantt_start, "hours") /
      gt.config.step *
      gt.config.column_width;

    if (gt.view_is("Month")) {
      x =
        self.task._start.diff(gt.gantt_start, "days") *
        gt.config.column_width /
        30;
    }
    return x;
  ...
  function update_progressbar_position() {
    ...
    self.$bar_progress.attr(
      "width",
      self.$bar.getWidth() * (self.task.progress / 100)
    );
  }
  ...
  function update_handle_position() {
    ...
    self.group
      .select(".handle.progress")
      .attr("points", get_progress_polygon_points());
  }
